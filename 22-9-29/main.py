...
class sgenerator:
    def __init__(self):
        ...
        debug_print('GENERATING PINYIN ANALYZER : ', end=' ')
        ...
        debug_print('GENERATING PROBABILITY : ', end=' ')
    ...
    def gen_matrix(self):
        ...
        debug_print('GENERATED PROBABILITY MATRIX : ')
    ...
    def gen_route(self, i, last, route: list):
        ...
        if i == len(self.matrix):
            ...
            '''
            debug_print('--'*i+'Route Generated : '+str(ret))
            return ret
        for j in range(len(self.matrix[i])):
            if self.matrix[i][j][2] == last:
                debug_print('--'*(i+1) + 'Generating: ' + str(ret) + ' + ' + str(j) )
                ret.append(j)
                # debug_print('-'*i + str(self.matrix[i][j][0]))
                self.gen_route(i+1, j, ret)
                ret.pop()

    def gen_sentence(self):
        """
        Select and return the text of a route with maximum P .
        :return:
        """
        debug_print('ROUTE GENERATED . ALL THE ROUTES :\n\t'+str(self.routes))
        ret = []
        ...
        debug_print('COMPUTING ROUTE PROBABILITIES ... \nCOMPUTING DONE . P : '+str(ret))
        max_p = 0
        tmp = []
        for item in ret:
            if float(item[1]) >= max_p:
                ...
                max_p = float(item[1])
        if DEBUG_MODE:
            print('THE BIGGEST P IN THE LIST IS : ',end='')
            print(tmp)
        else:
            print(tmp[0])
        return tmp
...
if __name__ == "__main__":
    while True:
        sgen = sgenerator()
        sgen.set_input(input('Type:'))
        sgen.gen_matrix()
        debug_print('ROUTE GENERATING:')
        debug_print('\t[EXISTED ROUTE] + ADDING NEW POINT')
        sgen.gen_route(0, 0, [])
        sgen.gen_sentence()
